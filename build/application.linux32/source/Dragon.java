...
public PVector randomPointAwayFromEdges() {
...
public boolean pointRectCollision(float px, float py, float rx, float ry, float rw, float rh) {
  return px >= rx && px <= rx + rw && py >= ry && py <= ry + rh;
}

public float normalizedDuration(float start, float duration) {
...
class Particle {
  ...
  public float getAngleAt(float x, float y) {
  ...
  public float angleBetween(float x0, float y0, float x1, float y1) {
...
class Obstacle {
  ...
  Obstacle() {
    ...
    if (random(1) > .5f) {
      ...
      float xSpd = x > width * .5f ? random(-maxSpd, 0) : random(0, maxSpd);
      ...
      //go to the right if you spawn on the left and vice versa
      float ySpd = y > height * .5f ? random(-maxSpd, 0) : random(0, maxSpd);
  ...
  public boolean isCollidingWithSnake() {
    if (snake.isInvulnerable()) {
      return false;
    }
...
class Resources {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Dragon" };
    if (passedArgs != null) {
